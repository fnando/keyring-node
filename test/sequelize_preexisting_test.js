...
async function defineModel({
  ...
  const encKeys = _.reduce(columns, (acc, column) => {
    acc[column] = keys;
    return acc;
  }, {})
  const keyringIdColumns = _.reduce(columns, (acc, column) => {
    acc[column] = keyringIdColumn;
    return acc;
  }, {})

  Keyring(model, { keys: encKeys, columns, encryption, keyringIdColumns, digestSalt });
